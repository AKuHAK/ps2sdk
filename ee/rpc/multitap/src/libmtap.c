...
int mtapInit(void)
{
    ...
    while (1) {
        if (SifBindRpc(&clientPortOpen, MTAPSERV_PORT_OPEN, 0) < 0)
            return -1;
        if (clientPortOpen.server != 0)
            break;
    ...
    while (1) {
        if (SifBindRpc(&clientPortClose, MTAPSERV_PORT_CLOSE, 0) < 0)
            return -1;
        if (clientPortClose.server != 0)
            break;
    ...
    while (1) {
        if (SifBindRpc(&clientGetConnection, MTAPSERV_GET_CONNECTION, 0) < 0)
            return -1;
        if (clientGetConnection.server != 0)
            break;
