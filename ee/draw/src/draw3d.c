...
qword_t *draw_prim_start(qword_t *q, int context, prim_t *prim, color_t *color)
{
    ...
    q++;
    PACK_GIFTAG(q, GS_SET_PRIM(prim->type, prim->shading, prim->mapping, prim->fogging, prim->blending, prim->antialiasing, prim->mapping_type, context, prim->colorfix), GS_REG_PRIM);
    q++;
