...
qword_t *draw_texturebuffer(qword_t *q, int context, texbuffer_t *texture, clutbuffer_t *clut)
{
    ...
    PACK_GIFTAG(q, GS_SET_TEX0(texture->address >> 6, texture->width >> 6, texture->psm, texture->info.width, texture->info.height, texture->info.components, texture->info.function, clut->address >> 6, clut->psm, clut->storage_mode, clut->start, clut->load_method), GS_REG_TEX0 + context);
    q++;
...
qword_t *draw_dither_matrix(qword_t *q, char *dm)
{
    ...
    PACK_GIFTAG(q, GS_SET_DIMX(dm[0], dm[1], dm[2], dm[3], dm[4], dm[5], dm[6], dm[7], dm[8], dm[9], dm[10], dm[11], dm[12], dm[13], dm[14], dm[15]), GS_REG_DIMX);
    q++;
...
qword_t *draw_primitive_override_setting(qword_t *q, int context, prim_t *prim)
{
    ...
    PACK_GIFTAG(q, GS_SET_PRMODE(prim->shading, prim->mapping, prim->fogging, prim->blending, prim->antialiasing, prim->mapping_type, context, prim->colorfix), GS_REG_PRMODE);
    q++;
...
qword_t *draw_mipmap1(qword_t *q, int context, mipmap_t *mipmap)
{
    ...
    PACK_GIFTAG(q, GS_SET_MIPTBP1(mipmap->address1, mipmap->width1, mipmap->address2, mipmap->width2, mipmap->address3, mipmap->width3), GS_REG_MIPTBP1 + context);
    q++;
...
qword_t *draw_mipmap2(qword_t *q, int context, mipmap_t *mipmap)
{
    ...
    PACK_GIFTAG(q, GS_SET_MIPTBP2(mipmap->address1, mipmap->width1, mipmap->address2, mipmap->width2, mipmap->address3, mipmap->width3), GS_REG_MIPTBP2 + context);
    q++;
...
qword_t *draw_pixel_test(qword_t *q, int context, atest_t *atest, dtest_t *dtest, ztest_t *ztest)
{
    ...
    PACK_GIFTAG(q, GS_SET_TEST(atest->enable, atest->method, atest->compval, atest->keep, dtest->enable, dtest->pass, ztest->enable, ztest->method), GS_REG_TEST + context);
    q++;
...
qword_t *draw_texture_wrapping(qword_t *q, int context, texwrap_t *wrap)
{
    ...
    PACK_GIFTAG(q, GS_SET_CLAMP(wrap->horizontal, wrap->vertical, wrap->minu, wrap->maxu, wrap->minv, wrap->maxv), GS_REG_CLAMP + context);
    q++;
