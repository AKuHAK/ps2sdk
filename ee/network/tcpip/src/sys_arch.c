...
static void SendMbx(sys_mbox_t *mbox, arch_message *msg, void *sys_msg)
{
    ...
    if ((*mbox)->FirstMessage == NULL)
        (*mbox)->FirstMessage = msg; // If this is the first message, it goes at the front.
    if ((*mbox)->LastMessage != NULL)
        (*mbox)->LastMessage->next = msg; // Otherwise, it becomes the next message of the last message.
