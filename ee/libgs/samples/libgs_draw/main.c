...
const static GS_RGBAQ randcolor[10] = {
...
static int InitSprites(void)
{
    ...
    for (i = 0, j = 0; i < MAX_SPRITES; i++, j++) {
        ...
        if (sprites[i].x_speed <= 0)
            sprites[i].x_speed = 2;
        if (sprites[i].y_speed <= 0)
            sprites[i].y_speed = 1;
...
static int MoveSprites(void)
{
    ...
    for (i = 0; i < MAX_SPRITES; i++) {
        ...
        if (sprites[i].x_speed > 10)
            sprites[i].x_speed = sprites[i].x_speed - 10;
        if (sprites[i].y_speed > 10)
            sprites[i].y_speed = sprites[i].y_speed - 10;
