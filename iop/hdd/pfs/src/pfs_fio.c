...
static int openFile(pfs_mount_t *pfsMount, pfs_file_slot_t *freeSlot, const char *filename, int openFlags, int mode)
{
    ...
    if (fileInode) {
        ...
        if (openFlags & FIO_O_TRUNC)
            flags |= 2;
        if (openFlags & PFS_FDIRO)
            flags |= 4;
        if ((mode & 0x10000) ||
...
static int fileTransfer(pfs_file_slot_t *fileSlot, u8 *buf, int size, int operation)
{
    ...
    while (size) {
        ...
        if (bytes_remain == 0) {
            ...
            } else if ((result = pfsBlockSeekNextSegment(fileSlot->clink, blockpos))) {
                return result;
            }
...
int pfsFioRename(iomanX_iop_file_t *ff, const char *old, const char *new)
{
    ...
    if (parentOld) {
        ...
        if ((iFileOld = pfsInodeGetFileInDir(parentOld, path1, &result)) == NULL)
            goto exit;

        if ((parentNew = pfsInodeGetParent(pfsMount, NULL, new, path2, &result)) == NULL)
            goto exit;
        ...
        if (iFileNew) {
            if (f) {
                if ((iFileNew->u.inode->mode & FIO_S_IFMT) != FIO_S_IFDIR)
                    result = -ENOTDIR;
                else if (pfsCheckDirForFiles(iFileNew)) {
                    if (iFileNew->nused >= 2)
                        result = -EBUSY;
                    else {
                    ...
                    result = -ENOTEMPTY;
            } else if ((iFileNew->u.inode->mode & FIO_S_IFMT) == FIO_S_IFDIR)
                result = -EISDIR;
            else if (iFileNew->nused >= 2)
                result = -EBUSY;
            else {
                if (iFileOld == iFileNew)
                    goto exit;
        ...
        if (result) {
            if (removeNew)
                removeNew->pfsMount = NULL;
            if (removeOld)
                removeOld->pfsMount = NULL;
            if (addNew)
                addNew->pfsMount = NULL;
            if (iFileNew)
                iFileNew->pfsMount = NULL;
        ...
        if (removeOld)
            pfsCacheFree(removeOld);
        if (addNew)
            pfsCacheFree(addNew);
        if (removeNew)
            pfsCacheFree(removeNew);
    exit:
        if (iFileNew)
            pfsCacheFree(iFileNew);
