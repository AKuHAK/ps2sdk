...
u32 PadIsSupported(padState_t *pstate)
{
    ...
    if (res == 0) {
        ...
        if ((pstate->outbuffer[1] != 0) && (sio2cmdCheckId(pstate->outbuffer[1]) == 1)) { // Twice? Okay...
            if ((pstate->outbuffer[1] != 0) && (sio2cmdCheckId(pstate->outbuffer[1]) == 1)) {
...
u32 QueryModel(padState_t *pstate)
{
    ...
    if (pdGetError(pstate->port, pstate->slot) == 0) {
        ...
        if (pstate->outbuffer[1] == PAD_ID_CONFIG) {
            ...
            if (pstate->numActuators > 4)
                pstate->numActuators = 4;
            if (pstate->numActComb > 4)
                pstate->numActComb = 4;
            if (pstate->numModes > 4)
                pstate->numModes = 4;
