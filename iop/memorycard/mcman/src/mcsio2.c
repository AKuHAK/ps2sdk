...
int mcsio2_transfer(int port, int slot, sio2_transfer_data_t *sio2data)
{
	...
	if (p)
		M_DEBUG("mcsio2_transfer port%d slot%d cmd = %02x %02x %02x ", port, slot, p[0], p[1], p[2]);
	else {
		p = (u8 *)(sio2data->in);
		M_DEBUG("mcsio2_transfer for secrman port%d slot%d cmd = %02x %02x %02x ", port, slot, p[0], p[1], p[2]);
	}
#endif
...
int mcsio2_transfer2(int port, int slot, sio2_transfer_data_t *sio2data)
{
	...
	M_DEBUG("mcsio2_transfer2 port%d slot%d\n", port, slot);
	...
	M_DEBUG("mcsio2_transfer2 returns %d\n", r);
...
void mcman_initPS2com(void)
{
	...
	M_DEBUG("mcman_initPS2com registering secrman_mc_command callback\n");
	SecrSetMcCommandHandler((void *)secrman_mc_command);

	M_DEBUG("mcman_initPS2com registering mcman_getcnum callback\n");
...
int secrman_mc_command(int port, int slot, sio2_transfer_data_t *sio2data)
{
	...
	M_DEBUG("secrman_mc_command port%d slot%d\n", port, slot);
...
int mcman_cardchanged(int port, int slot)
{
	...
	M_DEBUG("mcman_cardchanged sio2cmd port%d slot%d\n", port, slot);
	...
	if (retries >= 5) {
		M_DEBUG("mcman_cardchanged sio2cmd card changed!\n");
		return sceMcResChangedCard;
	}

	M_DEBUG("mcman_cardchanged sio2cmd succeeded\n");
#endif
...
int McGetCardSpec(int port, int slot, s16 *pagesize, u16 *blocksize, int *cardsize, u8 *flags)
{
	...
	M_DEBUG("McGetCardSpec sio2cmd port%d slot%d\n", port, slot);
	...
	*pagesize = 512;
	...
	M_DEBUG("McGetCardSpec sio2cmd pagesize=%d blocksize=%u cardsize=%d flags%x\n", *pagesize, *blocksize, *cardsize, *flags);
...
int mcman_resetauth(int port, int slot)
{
	...
	M_DEBUG("mcman_resetauth sio2cmd port%d slot%d\n", port, slot);
	...
	if (retries >= 5) {
		M_DEBUG("mcman_resetauth sio2cmd card changed!\n");

		return sceMcResChangedCard;
	}

	M_DEBUG("mcman_resetauth sio2cmd succeeded\n");
#endif
...
int mcman_probePS2Card2(int port, int slot)
{
	...
	M_DEBUG("mcman_probePS2Card2 sio2cmd port%d slot%d\n", port, slot);
	...
	if (p[3] == 0x5a) {
		...
		if (r > 0) {
			M_DEBUG("mcman_probePS2Card2 succeeded\n");
		...
		r = McGetFormat(port, slot);
		if (r < 0) {
			M_DEBUG("mcman_probePS2Card2 sio2cmd failed (no format)\n");

			return sceMcResNoFormat;
		}
	}

	M_DEBUG("mcman_probePS2Card2 sio2cmd failed (mc detection failed)\n");
#endif
#ifdef BUILDING_XFROMMAN
...
int mcman_probePS2Card(int port, int slot) //2
{
	...
	M_DEBUG("mcman_probePS2Card sio2cmd port%d slot%d\n", port, slot);

	r = mcman_cardchanged(port, slot);
	if (r == sceMcResSucceed) {
		r = McGetFormat(port, slot);
		if (r != 0) {
			M_DEBUG("mcman_probePS2Card sio2cmd succeeded\n");
	...
	if (mcman_resetauth(port, slot) != sceMcResSucceed) {
		M_DEBUG("mcman_probePS2Card sio2cmd failed (auth reset failed)\n");

		return sceMcResFailResetAuth;
	}

	if (SecrAuthCard(port + 2, slot, mcman_getcnum(port, slot)) == 0) {
		M_DEBUG("mcman_probePS2Card sio2cmd failed (auth failed)\n");
	...
	if (retries >= 5) {
		M_DEBUG("mcman_probePS2Card sio2cmd failed (mc detection failed)\n");
		return sceMcResFailDetect;
	}
#endif
	...
	if (r == 0) {
		M_DEBUG("mcman_probePS2Card sio2cmd card changed!\n");
		return sceMcResChangedCard;
	}
...
int mcman_probePS1Card2(int port, int slot)
{
	...
	M_DEBUG("mcman_probePS1Card2 port%d slot%d\n", port, slot);
...
int mcman_probePS1Card(int port, int slot)
{
	...
	M_DEBUG("mcman_probePS1Card port%d slot%d\n", port, slot);
...
int mcman_probePDACard(int port, int slot)
{
	...
	M_DEBUG("mcman_probePDACard port%d slot%d\n", port, slot);
...
int McWritePS1PDACard(int port, int slot, int page, void *buf) // Export #30
{
	...
	// M_DEBUG("McWritePS1PDACard port%d slot%d page %x\n", port, slot, page);
...
int McReadPS1PDACard(int port, int slot, int page, void *buf) // Export #29
{
	...
	// M_DEBUG("McReadPS1PDACard port%d slot%d page %x\n", port, slot, page);
