...
int mcman_setdevinfos(int port, int slot)
{
    ...
    while (cluster_cnt < allocatable_clusters_per_card) {
        ...
        if (((current_allocatable_cluster % mcdi->clusters_per_block) == 0) || (mcdi->alloc_offset == (u32)current_allocatable_cluster)) {
            iscluster_valid = 1;
            for (r = 0; r < 16; r++) {
...
int McCreateDirentry(int port, int slot, int parent_cluster, int num_entries, int cluster, const sceMcStDateTime *ctime)
{
    ...
    mfe->mode = sceMcFileAttrReadable | sceMcFileAttrWriteable | sceMcFileAttrExecutable | sceMcFileAttrSubdir | sceMcFile0400 | sceMcFileAttrExists; // 0x8427
    ...
    if ((parent_cluster == 0) && (num_entries == 0)) {
        ...
        mfe->mode      = sceMcFileAttrWriteable | sceMcFileAttrExecutable | sceMcFileAttrSubdir | sceMcFile0400 | sceMcFileAttrExists | sceMcFileAttrHidden; // 0xa426
        mfe->dir_entry = 0;
        mfe->cluster   = 0;
    } else {
        // entry is normal "." / ".."
