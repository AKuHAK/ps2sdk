...
int _start(int argc, const char **argv)
{
	...
	if (!sio2man_loaded) {
		M_DEBUG("sio2man module is not loaded...\n");
		return MODULE_NO_RESIDENT_END;
	}

	M_DEBUG("sio2man version=0x%03x\n", libptr->version);
...
{
	M_DEBUG("McGetFormat port%d slot%d\n", port, slot);
...
{
	M_DEBUG("McGetMcType port%d slot%d\n", port, slot);
...
{
	M_DEBUG("McSetPS1CardFlag flag %x\n", flag);
	PS1CardFlag = flag;
}
...
int McGetFreeClusters(int port, int slot) // Export #38
{
	...
	M_DEBUG("McGetFreeClusters port%d slot%d\n", port, slot);
...
int mcman_detectcard(int port, int slot)
{
	...
	M_DEBUG("mcman_detectcard port%d slot%d\n", port, slot);
...
int McDetectCard2(int port, int slot) // Export #21 XMCMAN only
{
	...
	M_DEBUG("McDetectCard2 port%d slot%d\n", port, slot);
...
int McSeek(int fd, int offset, int origin) // Export #10
{
	...
	M_DEBUG("McSeek fd %d offset %d origin %d\n", fd, offset, origin);
...
int mcman_setdevspec(int port, int slot)
{
	...
	M_DEBUG("mcman_setdevspec port%d, slot%d\n", port, slot);
...
int mcman_setdevinfos(int port, int slot)
{
	...
	M_DEBUG("mcman_setdevinfos port%d slot%d\n", port, slot);
	...
	if (strncmp(SUPERBLOCK_MAGIC, mcman_pagebuf.magic, 28) != 0) {
		M_DEBUG("mcman_setdevinfos No card format !!!\n");
...
int mcman_reportBadBlocks(int port, int slot)
{
	...
	M_DEBUG("mcman_reportBadBlocks port%d, slot%d\n", port, slot);
...
int McCreateDirentry(int port, int slot, int parent_cluster, int num_entries, int cluster, sceMcStDateTime *ctime)
{
	...
	M_DEBUG("McCreateDirentry port%d slot%d parent_cluster %x num_entries %d cluster %x\n", port, slot, parent_cluster, num_entries, cluster);
...
int mcman_findfree2(int port, int slot, int reserve)
{
	...
	M_DEBUG("mcman_findfree2 port%d slot%d reserve%d\n", port, slot, reserve);
...
int mcman_getentspace(int port, int slot, char *dirname)
{
	...
	M_DEBUG("mcman_getentspace port%d slot%d dirname %s\n", port, slot, dirname);
...
int mcman_cachedirentry(int port, int slot, char *filename, McCacheDir *pcacheDir, McFsEntry **pfse, int unknown_flag)
{
	...
	M_DEBUG("mcman_cachedirentry port%d slot%d name %s\n", port, slot, filename);
...
int mcman_getdirinfo(int port, int slot, McFsEntry *pfse, char *filename, McCacheDir *pcd, int unknown_flag)
{
	...
	M_DEBUG("mcman_getdirinfo port%d slot%d name %s\n", port, slot, filename);
...
int McCheckBlock(int port, int slot, int block)
{
	...
	M_DEBUG("McCheckBlock port%d slot%d block 0x%x\n", port, slot, block);
...
int mcman_setPS1devinfos(int port, int slot)
{
	...
	M_DEBUG("mcman_setPS1devinfos port%d slot%d\n", port, slot);
...
int mcman_getPS1direntry(int port, int slot, char *filename, McFsEntryPS1 **pfse, int flag)
{
	...
	M_DEBUG("mcman_getPS1direntry port%d slot%d file %s flag %x\n", port, slot, filename, flag);
...
int mcman_clearPS1direntry(int port, int slot, int cluster, int flags)
{
	...
	M_DEBUG("mcman_clearPS1direntry port%d slot%d cluster %x flags %x\n", port, slot, cluster, flags);
...
int mcman_findfree1(int port, int slot, int reserve)
{
	...
	M_DEBUG("mcman_findfree1 port%d slot%d reserve %d\n", port, slot, reserve);
...
int mcman_FNC8ca4(int port, int slot, MC_FHANDLE *fh)
{
	...
	M_DEBUG("mcman_FNC8ca4 port%d slot%d\n", port, slot);
...
int mcman_cachePS1dirs(int port, int slot)
{
	...
	M_DEBUG("mcman_cachePS1dirs port%d slot%d\n", port, slot);
...
int mcman_clearcache(int port, int slot)
{
	...
	M_DEBUG("mcman_clearcache port%d, slot%d\n", port, slot);
...
McCacheEntry *mcman_getcacheentry(int port, int slot, int cluster)
{
	...
	// M_DEBUG("mcman_getcacheentry port%d slot%d cluster %x\n", port, slot, cluster);
...
int McFlushCache(int port, int slot)
{
	...
	M_DEBUG("McFlushCache port%d slot%d\n", port, slot);
...
int mcman_flushcacheentry(McCacheEntry *mce)
{
	...
	M_DEBUG("mcman_flushcacheentry mce %x cluster %x\n", (int)mce, (int)mce->cluster);
...
lbl0:
			...
			do {
				if (((mce->wr_flag >> j) & 1) != 0) {
					// M_DEBUG("flushing ent %x cl_data %x to page %d offset %d\n", mce, mce->cl_data, i, (pagesize * j));
...
int McReadDirEntry(int port, int slot, int cluster, int fsindex, McFsEntry **pfse) // Export #47 XMCMAN only
{
	...
	M_DEBUG("McReadDirEntry port%d slot%d cluster %d fsindex %d\n", port, slot, cluster, fsindex);
...
int McSetFATentry(int port, int slot, int fat_index, int fat_entry) // Export #46 XMCMAN only
{
	...
	//M_DEBUG("McSetFATentry port%d slot%d fat_index %x fat_entry %x\n", port, slot, fat_index, fat_entry);
...
int mcman_fillbackupblock1(int port, int slot, int block, void **pagedata, void *eccdata)
{
	...
	M_DEBUG("mcman_fillbackupblock1 port%d slot%d block %x mcman_badblock %x\n", port, slot, block, mcman_badblock);
...
int McReplaceBadBlock(void)
{
	...
	M_DEBUG("McReplaceBadBlock mcman_badblock_port%d mcman_badblock_slot%d mcman_badblock %x\n", mcman_badblock_port, mcman_badblock_slot, mcman_badblock);
