...
int mcman_format2(int port, int slot)
{
	...
	M_DEBUG("mcman_format2 port%d, slot%d cardform %d\n", port, slot, mcdi->cardform);
...
int mcman_getstat2(int port, int slot, char *filename, io_stat_t *stat)
{
	...
	M_DEBUG("mcman_getstat2 port%d slot%d filename %s\n", port, slot, filename);
...
int mcman_setinfo2(int port, int slot, char *filename, sceMcTblGetDir *info, int flags)
{
	...
	M_DEBUG("mcman_setinfo2 port%d slot%d filename %s flags %x\n", port, slot, filename, flags);
...
int mcman_read2(int fd, void *buffer, int nbyte)
{
	...
	M_DEBUG("mcman_read2 fd %d buf %x size %d\n", fd, (int)buffer, nbyte);
...
int mcman_open2(int port, int slot, char *filename, int flags)
{
	...
	M_DEBUG("mcman_open2 port%d slot%d name %s flags %x\n", port, slot, filename, flags);
...
int mcman_chdir(int port, int slot, char *newdir, char *currentdir)
{
	...
	M_DEBUG("mcman_chdir port%d slot%d newdir %s\n", port, slot, newdir);
...
int mcman_getdir2(int port, int slot, char *dirname, int flags, int maxent, sceMcTblGetDir *info)
{
	...
	M_DEBUG("mcman_getdir2 port%d slot%d dir=%s flags=%d maxent=%d\n", port, slot, dirname, flags, maxent);
...
int mcman_delete2(int port, int slot, char *filename, int flags)
{
	...
	M_DEBUG("mcman_delete2 port%d slot%d filename %s flags %x\n",  port, slot, filename, flags);
...
int mcman_unformat2(int port, int slot)
{
	...
	M_DEBUG("mcman_unformat2 port%d slot%d\n", port, slot);
