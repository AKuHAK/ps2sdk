...

{
    ...
    if (dev != NULL) {
        ...
        if (mouse_readmode == PS2MOUSE_READMODE_ABS) {
            if (mouse.x < mousex_min)
                mouse.x = mousex_min;
            if (mouse.x > mousex_max)
                mouse.x = mousex_max;
            if (mouse.y < mousey_min)
                mouse.y = mousey_min;
            if (mouse.y > mousey_max)
                mouse.y = mousey_max;
        }
...

{
    switch (command) {
        case PS2MOUSE_READ:
            do_ps2mouse_read((u8 *)buffer, size);
            break;
        case PS2MOUSE_SETREADMODE:
            do_ps2mouse_setreadmode((u32 *)buffer, size);
            break;
        case PS2MOUSE_GETREADMODE:
            do_ps2mouse_getreadmode((u32 *)buffer, size);
            break;
        case PS2MOUSE_SETTHRES:
            do_ps2mouse_setthres((u32 *)buffer, size);
            break;
        case PS2MOUSE_GETTHRES:
            do_ps2mouse_getthres((u32 *)buffer, size);
            break;
        case PS2MOUSE_SETACCEL:
            do_ps2mouse_setaccel((u32 *)buffer, size);
            break;
        case PS2MOUSE_GETACCEL:
            do_ps2mouse_getaccel((u32 *)buffer, size);
            break;
        case PS2MOUSE_SETBOUNDARY:
            do_ps2mouse_setboundary((s32 *)buffer, size);
            break;
        case PS2MOUSE_GETBOUNDARY:
            do_ps2mouse_getboundary((s32 *)buffer, size);
            break;
        case PS2MOUSE_SETPOSITION:
            do_ps2mouse_setposition((s32 *)buffer, size);
            break;
        case PS2MOUSE_RESET:
            do_ps2mouse_reset();
            break;
        case PS2MOUSE_ENUM:
            do_ps2mouse_enum((u32 *)buffer, size);
            break;
        case PS2MOUSE_SETDBLCLICKTIME:
            do_ps2mouse_setdblclicktime((u32 *)buffer, size);
            break;
        case PS2MOUSE_GETDBLCLICKTIME:
            do_ps2mouse_getdblclicktime((u32 *)buffer, size);
            break;
        case PS2MOUSE_GETVERSION:
            do_ps2mouse_getversion((u32 *)buffer, size);
            break;
        default:
            printf("Unknown PS2MOUSE command %ld\n", command);
            break;
    }
