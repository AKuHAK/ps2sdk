...
#endif
{
    ...
    if (dev != NULL) {
        ...
        if (((subdev == 0) && (name[name_len + fn_offset] != '0')) || (subdev >= DEVFS_MAX_SUBDEVS))
        /* Invalid number */
        {
        ...
        if ((dev->subdevs[subdev].mode & DEVFS_MODE_EX) && (dev->subdevs[subdev].open_refcount > 0))
        ...
        if (((mode & O_RDONLY) || ((mode & O_RDWR) == O_RDWR)) && !(dev->subdevs[subdev].mode & DEVFS_MODE_R)) {
            M_PRINTF("open: Read mode requested but not permitted\n");
            return -1;
        }

        if (((mode & O_WRONLY) || ((mode & O_RDWR) == O_RDWR)) && !(dev->subdevs[subdev].mode & DEVFS_MODE_W)) {
...

{
    ...
    if ((data) && (data->hDev != INVALID_HDEV)) /* Invalid HDEV is a dir listing */
    {
        ...
        if (dev != NULL) {
            ...
            loop = 0;
            while ((loop < MAX_OPENFILES) && (dev->subdevs[data->subdev].open_files[loop] != data)) {
                loop++;
            }
...

{
    ...
    if ((data) && (data->hDev != INVALID_HDEV)) {
        ...
        if ((dev->node.read != NULL) && (dev->subdevs[data->subdev].mode & DEVFS_MODE_R)) {
            devfs_info_t dev_info;
            int bytes_read;
...

{
    ...
    if ((data) && (data->hDev != INVALID_HDEV)) {
        ...
        if ((dev->node.write != NULL) && (dev->subdevs[data->subdev].mode & DEVFS_MODE_W)) {
            devfs_info_t dev_info;
            int bytes_written;
    ...
    if ((data) && (data->hDev != INVALID_HDEV)) {
        ...
        switch (whence) {
            ...
            case SEEK_CUR:
                ...
                break;
            case SEEK_END:
                if ((loc > 0) && (dev->subdevs[data->subdev].extent.loc64 >= (u64)loc))
