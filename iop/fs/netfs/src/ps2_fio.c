...
static inline int convmode_to_iomanx(int stat)
{
    ...
    if (FIO_SO_ISLNK(stat))
        mode |= FIO_S_IFLNK; // Symbolic link
    if (FIO_SO_ISREG(stat))
        mode |= FIO_S_IFREG; // regular file
    if (FIO_SO_ISDIR(stat))
        mode |= FIO_S_IFDIR; // directory
...
static inline int ps2netfs_lwip_send(int sock, void *buf, int len, int flag)
{
    ...
    if (ret < 0) {
        ...
        return -1;
    } else if (len == 0) { // send is blocking, so 0 means error
...
int ps2netfs_recv_bytes(int sock, char *buf, int bytes)
{
    ...
    while (left > 0) {
        ...
        if (len == 0) { // recv is blocking, so 0 means error
...
static int ps2netfs_op_read(char *buf, int len)
{
    ...
    if (nbytes > FIOTRAN_MAXSIZE)
        nbytes = FIOTRAN_MAXSIZE;
...
static int ps2netfs_op_write(char *buf, int len)
{
    ...
    if (fdptr != 0) {
        ...
        while ((retval >= 0) && (left > 0)) {
            ...
            towrite = left;
            if (towrite > FIOTRAN_MAXSIZE)
                towrite = FIOTRAN_MAXSIZE;
...
static int ps2netfs_op_format(char *buf, int len)
{
    ...
    if (devtype == IOPMGR_DEVTYPE_IOMANX) { // if other end has not set args, add some standard ones (hdd use only for mo)
        if (ntohl(cmd->arglen) == 0) {
            int Arg[3] = {PFS_ZONE_SIZE, 0x2d66, PFS_FRAGMENT};
            retval     = format(cmd->dev, cmd->blockdev, (char *)&Arg, sizeof(Arg));
        } else
            retval = format(cmd->dev, cmd->blockdev, cmd->arg, ntohl(cmd->arglen));
    }
