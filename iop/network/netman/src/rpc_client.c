...
int NetManInitRPCClient(void)
{
    ...
    if (FrameBufferStatus == NULL)
        FrameBufferStatus = malloc(NETMAN_RPC_BLOCK_SIZE * sizeof(struct NetManBD));
...
static void EnQFrame(const void *frame, unsigned int length)
{
    ...
    if (NumFramesInQueue >= NETMAN_FRAME_GROUP_SIZE) { /* If there are already sufficient frames, the frames can be sent right away.
    ...
    if (NumFramesInQueue >= NETMAN_FRAME_GROUP_SIZE) { // If there are sufficient frames, the frames can be sent right away.
        sendFramesToEE(0);
    } else {
